    ...
    fn parse() -> Result<Args> {
        ...
        let mut app = App::new("List files using walkdir")
            ...
            .arg(Arg::with_name("file-size")
                 .long("file-size")
                 .help("Print the total file size of all files. This \
                        implies --count.")
            )
            .arg(Arg::with_name("flat-std")
                 .long("flat-std")
                 .conflicts_with("flat-unix")
                 .conflicts_with("flat-linux")
                 .conflicts_with("flat-windows")
                 .help("Use std::fs::read_dir to list contents of a single \
                        directory. This is NOT recursive.")
            );
        if cfg!(unix) {
            app = app
                .arg(Arg::with_name("flat-unix")
                .long("flat-unix")
                .conflicts_with("flat-std")
                .conflicts_with("flat-linux")
                .conflicts_with("flat-windows")
                .help("Use Unix-specific APIs to list contents of a single \
                       directory. This is NOT recursive."));
        }
        if cfg!(target_os = "linux") {
            app = app
                .arg(Arg::with_name("flat-linux")
                .long("flat-linux")
                .conflicts_with("flat-std")
                .conflicts_with("flat-unix")
                .conflicts_with("flat-windows")
                .help("Use Linux-specific syscalls (getdents64) to list \
