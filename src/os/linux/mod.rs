...
use libc::{SYS_getdents64, syscall};

use os::linux::dirent::RawDirEntry;
use os::unix::{
    DirFd, DirEntry as UnixDirEntry, FileType,
    errno, escaped_bytes,
};
...
impl DirEntryCursor {
    ...
    pub fn read<'a>(&'a mut self) -> Option<DirEntry<'a>> {
        ...
        // for RawDirEntry.
        let raw_dirent = unsafe {
            &*(self.cursor.as_ptr() as *const RawDirEntry)
        };
        let ent = DirEntry {
