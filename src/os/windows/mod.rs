...
impl DirEntry {
    ...
    fn from_find_data(&mut self, fd: &FindData) {
        ...
        fd.decode_file_name_into(&mut self.file_name);
        fd.read_file_name_utf16(&mut self.file_name_u16);
    }
    ...
    /// predicates, and the rest of the public API functions on a `DirEntry`
    /// return `0`.
    ///
...
/// [available in `winapi-util` crate](https://docs.rs/winapi-util/0.1.2/x86_64-pc-windows-msvc/winapi_util/file/fn.typ.html).
#[derive(Clone, Copy)]
pub struct FileType {
    attr: DWORD,
    reparse_tag: DWORD,
...
impl FindHandle {
    ...
    pub fn close(self) -> io::Result<()> {
        let res =
            if unsafe { FindClose(self.handle) } == 0 {
                Err(io::Error::last_os_error())
            } else {
                Ok(())
            };
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("FindData")
            ...
            .field(
                "cAlternateFileName",
                &OsString::from_wide(
                    &truncate_utf16(&self.0.cAlternateFileName),
                ),
            )
            .finish()
    }
...
impl FindData {
    ...
    fn file_name(&self) -> OsString {
        ...
        self.decode_file_name_into(&mut os);
        os
    }
    ...
    fn decode_file_name_into(&self, dst: &mut OsString) {
        ...
        for result in char::decode_utf16(file_name.iter().cloned()) {
            match result {
                ...
                Err(_) => {
                    *dst = OsString::from_wide(file_name);
                    return;
                }
            }
        }
    ...
    /// Read the contents of the file name as raw code units. Note that this
    /// may not be valid UTF-16.
    fn read_file_name_utf16(&self, dst: &mut Vec<u16>) {
        dst.extend_from_slice(truncate_utf16(&self.0.cFileName));
    }
}
