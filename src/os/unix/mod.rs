...
use os::unix::dirent::RawDirEntry;
#[cfg(target_os = "linux")]
use os::linux::DirEntry as LinuxDirEntry;
...
impl DirEntry {
    ...
    pub fn file_name(&self) -> &CStr {
        ...
        // easier.
        unsafe {
            CStr::from_bytes_with_nul_unchecked(&self.file_name)
        }
    }
    ...
    pub fn into_file_name(mut self) -> CString {
        ...
        unsafe {
            ...
            //
            // CString really should have a from_vec_with_nul_unchecked routine
            // like CStr.
...
impl DirFd {
    ...
    pub fn open_c(dir_path: &CStr) -> io::Result<DirFd> {
        ...
        let fd = unsafe {
            libc::open(dir_path.as_ptr(), flags)
        };
        if fd < 0 {
    ...
    pub fn openat<D: Into<OsString>>(
        ...
        DirFd::openat_c(
            parent_dirfd, &CString::new(dir_name.into().into_vec())?,
        )
    }
    ...
    pub fn openat_c(
        ...
        // interior NUL bytes and is terminated by a NUL.
        let fd = unsafe {
            libc::openat(parent_dirfd, dir_name.as_ptr(), flags)
        };
        if fd < 0 {
            Err(io::Error::last_os_error())
        } else {
...
impl Dir {
    ...
    pub fn close(self) -> io::Result<()> {
        let res =
            if unsafe { libc::closedir(self.0.as_ptr()) } < 0 {
                Err(io::Error::last_os_error())
            } else {
                Ok(())
            };
